...
def dump_imginfo(imgfile, outfile=None, silent=False):
    ...
    # Compatibility to .hex file
    try:
        ext = os.path.splitext(imgfile)[1][1:].lower()
        if ext == "hex":
            ih = IntelHex(imgfile)
            b=bytearray(ih.tobinarray())
        else:
    ...
    # Check the total length of the binary data
    print(f"Total length of binary data: {len(b)} bytes")

    # Check that the offset is valid
    if tlv_off + image.TLV_INFO_SIZE > len(b):
        raise ValueError(f"Offset {tlv_off} and size {image.TLV_INFO_SIZE} exceed the length of binary data")

    if protected_tlv_size != 0:
        data_to_unpack = b[tlv_off:(tlv_off + image.TLV_INFO_SIZE)]
        print(f"Data to unpack (expected {image.TLV_INFO_SIZE} bytes): {data_to_unpack}")
