...
{
#ifdef MCUBOOT_ENC_IMAGES
    memset32(&bs->enckey, 0xff, BOOT_NUM_SLOTS * BOOT_ENC_KEY_ALIGN_SIZE);
#if MCUBOOT_SWAP_SAVE_ENCTLV
    memset32(&bs->enctlv, 0xff, BOOT_NUM_SLOTS * BOOT_ENC_TLV_ALIGN_SIZE);
#endif
                   ...
                   struct boot_status *bs)
{
     ...
     * with other images using another magic. Additionally, no need to perform
     * image check in this case if header is not valid.
     */
    if (!boot_is_header_valid(hdr, fap))
    {
        fih_rc = FIH_NO_BOOTABLE_IMAGE;
        BOOT_LOG_INF("Image in the %s slot is not valid!",
                             ...
                             struct image_header *hdr, uint32_t *updated)
{
    ...
    if (*updated != 0) {
        BOOT_LOG_INF("Updated security counter %d %x", image_index, (unsigned int)img_security_cnt);
    }
...
boot_swap_image(struct boot_loader_state *state, struct boot_status *bs)
{
    ...
    if (boot_status_is_reset(bs)) {
        ...
        if (IS_ENCRYPTED(hdr)) {
            ...
            memset32(bs->enckey[0], 0xff, BOOT_ENC_KEY_ALIGN_SIZE);
        }
#endif
        ...
        if (IS_ENCRYPTED(hdr)) {
            ...
            memset32(bs->enckey[1], 0xff, BOOT_ENC_KEY_ALIGN_SIZE);
        }
#endif
...
boot_update_hw_rollback_protection(struct boot_loader_state *state)
{
    ...
    */
#if !defined(MCUBOOT_OVERWRITE_ONLY)
    /* When the 'revert' mechanism is enabled, the
    ...
    if ((swap_state.image_ok == BOOT_FLAG_SET) &&
         ...
         * process from begining with up to date security cnt */
        if (security_counter_updated) {
            BOOT_LOG_INF("Reset device");
            NVIC_SystemReset();
        }
    }
#endif /* !MCUBOOT_OVERWRITE_ONLY */
...
context_boot_go(struct boot_loader_state *state, struct boot_rsp *rsp)
{
     ...
     * completed.
     */
    BOOT_LOG_INF("Starting secondary slots verification");
    IMAGES_ITER(BOOT_CURR_IMG(state)) {
#if BOOT_IMAGE_NUMBER > 1
        if (state->img_mask[BOOT_CURR_IMG(state)]) {
            continue;
...
#if !defined(MCUBOOT_OVERWRITE_ONLY)
    /* Iterate over all the images. By the end of the loop, the security
     * counters will be up to date.
     * If a security counter update is performed, the system must reset because
     * swap type has to be re-evaluated.
     */
    IMAGES_ITER(BOOT_CURR_IMG(state)) {
    ...
    /* Iterate over all the images. Double check signature control result.
     */
    IMAGES_ITER(BOOT_CURR_IMG(state)) {
        if (BOOT_SWAP_TYPE(state) != BOOT_SWAP_TYPE_NONE) {
            if (ImageValidStatus[BOOT_CURR_IMG(state)] != IMAGE_VALID) {
                /* double signature check is not satisfied: erase candidate image
                 * Trig a FIH_PANIC to restart the system when FIH_ENABLE_GLOBAL_FAIL is defined
                 */
                boot_erase_secondary_slot(BOOT_CURR_IMG(state));
                FIH_PANIC;
            }
        }
    }
     ...
     */
    BOOT_LOG_INF("Starting primary slots validation");
    FIH_SET(fih_cnt, 0);
    IMAGES_ITER(BOOT_CURR_IMG(state)) {
#if BOOT_IMAGE_NUMBER > 1
        ...
        ImageValidStatus[BOOT_CURR_IMG(state)] = IMAGE_INVALID;
...
#ifdef MCUBOOT_OVERWRITE_ONLY
        /*
         * Erase cmplete secondary slot, if image has just been installed in primary
         * slot. This is done after revalidation of primary slot to ensure
         * installation is successful. The complete slot erase ensures new image
         * download can be performed without remaining data from previously
         * installed image.
         */
        image_index = BOOT_CURR_IMG(state);
