         ...
         uint32_t copy_size)
{
    ...
    BOOT_LOG_INF("Swapping secondary and primary slots: 0x%x bytes", (unsigned int)copy_size);
    ...
    while (last_sector_idx >= 0) {
        ...
        if (swap_idx >= (bs->idx - BOOT_STATUS_IDX_0)) {
            BOOT_LOG_INF("Swapping: swap index 0x%x, sector index 0x%x, size 0x%x",
                         (unsigned int)swap_idx, (unsigned int)first_sector_idx, (unsigned int)sz);
...
int app_max_size(struct boot_loader_state *state)
{
    ...
    (void)primary_slot_sz;
    (void)secondary_slot_sz;
