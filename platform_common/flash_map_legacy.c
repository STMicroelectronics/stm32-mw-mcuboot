...
int flash_area_get_sectors(int idx, uint32_t *cnt, struct flash_sector *ret)
{
    ...
    *cnt = 0;
    while (rem_len > 0 && *cnt < max_cnt) {
        if (rem_len < FLASH_SECTORS_SIZE) {
            BOOT_LOG_ERR("area %d size 0x%x not divisible by sector size 0x%x",
                         idx, (int)fa->fa_size, FLASH_SECTORS_SIZE);
            goto fa_close_out;
        }

        ret[*cnt].fs_off = FLASH_SECTORS_SIZE * (*cnt);
        ret[*cnt].fs_size = FLASH_SECTORS_SIZE;
        *cnt = *cnt + 1;
        rem_len -= FLASH_SECTORS_SIZE;
    }
